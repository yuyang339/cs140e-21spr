...
fld_t *staff_mmu_pt_alloc(unsigned n_entries);
...
void staff_write_domain_access_ctrl(unsigned d);
...
uint32_t staff_read_domain_access_ctrl(void);
...
static inline uint32_t mmu_sec_num(uint32_t u)       { return u >> 20; }
